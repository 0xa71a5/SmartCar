...
void ADC_Init(ADC_InitTypeDef* ADC_InitStruct)
{
    ...
    *(uint32_t*)SIM_ADCClockGateTable[ADC_InitStruct->instance].addr |= SIM_ADCClockGateTable[ADC_InitStruct->instance].mask;
    ...
    /* SingleOrDifferential */
	(kADC_Differential == ADC_InitStruct->singleOrDiffMode)?(ADC_InstanceTable[ADC_InitStruct->instance]->SC1[kADC_MuxA] |= ADC_SC1_DIFF_MASK):(ADC_InstanceTable[ADC_InitStruct->instance]->SC1[kADC_MuxA] &= ~ADC_SC1_DIFF_MASK);
